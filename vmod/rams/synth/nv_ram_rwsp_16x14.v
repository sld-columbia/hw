...
// nv_ram_rwsp_16x14: synthesizable model wrapper
...
// This wrapper consists of :  1 Ram cells: RAMDP_16X14_GL_M1_E2 ;  
...
nv_ram_rwsp_16x14_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_16x14 (
...
// initializing RAMDP_16X14_GL_M1_E2
for (row = 0; row < 16; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_16x14.ram_Inst_16X14.mem_write(row - 0, shadow_mem[row][13:0]);
...
// probing RAMDP_16X14_GL_M1_E2
 if (row >=  0 &&  row < 16) data[13:0] = r_nv_ram_rwsp_16x14.ram_Inst_16X14.mem_read(row - 0);
    probe_mem_val = data;
...
integer bitnum;
begin
  random_num = rflip.rollpli(0, 224);
  row = random_num / 14;
